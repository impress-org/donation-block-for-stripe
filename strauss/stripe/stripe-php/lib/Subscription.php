 ...
 * @property null|\GiveDonationBlock\Strauss\Stripe\StripeObject $billing_thresholds Define thresholds at which an invoice will be sent, and the subscription advanced to a new billing period
 ...
 * @property string|\GiveDonationBlock\Strauss\Stripe\Customer $customer ID of the customer who owns the subscription.
 ...
 * @property null|string|\GiveDonationBlock\Strauss\Stripe\Account|\GiveDonationBlock\Strauss\Stripe\AlipayAccount|\GiveDonationBlock\Strauss\Stripe\BankAccount|\GiveDonationBlock\Strauss\Stripe\BitcoinReceiver|\GiveDonationBlock\Strauss\Stripe\Card|\GiveDonationBlock\Strauss\Stripe\Source $default_source ID of the default payment source for the subscription. It must belong to the customer associated with the subscription and be in a chargeable state. If <code>default_payment_method</code> is also set, <code>default_payment_method</code> will take precedence. If neither are set, invoices will use the customer's <a href="https://stripe.com/docs/api/customers/object#customer_object-invoice_settings-default_payment_method">invoice_settings.default_payment_method</a> or <a href="https://stripe.com/docs/api/customers/object#customer_object-default_source">default_source</a>.
 * @property null|\GiveDonationBlock\Strauss\Stripe\TaxRate[] $default_tax_rates The tax rates that will apply to any subscription item that does not have <code>tax_rates</code> set. Invoices created will have their <code>default_tax_rates</code> populated from the subscription.
 * @property null|\GiveDonationBlock\Strauss\Stripe\Discount $discount Describes the current discount applied to this subscription, if there is one. When billing, a discount applied to a subscription overrides a discount applied on a customer-wide basis.
 * @property null|int $ended_at If the subscription has ended, the date the subscription ended.
 * @property \GiveDonationBlock\Strauss\Stripe\Collection $items List of subscription items, each with an attached price.
 * @property null|string|\GiveDonationBlock\Strauss\Stripe\Invoice $latest_invoice The most recent invoice this subscription has generated.
 ...
 * @property null|\GiveDonationBlock\Strauss\Stripe\StripeObject $pause_collection If specified, payment collection for this subscription will be paused.
 * @property null|\GiveDonationBlock\Strauss\Stripe\StripeObject $payment_settings Payment settings passed on to invoices created by the subscription.
 * @property null|\GiveDonationBlock\Strauss\Stripe\StripeObject $pending_invoice_item_interval Specifies an interval for how often to bill for any pending invoice items. It is analogous to calling <a href="https://stripe.com/docs/api#create_invoice">Create an invoice</a> for the given subscription at the specified interval.
 * @property null|string|\GiveDonationBlock\Strauss\Stripe\SetupIntent $pending_setup_intent You can use this <a href="https://stripe.com/docs/api/setup_intents">SetupIntent</a> to collect user authentication when creating a subscription without immediate payment or updating a subscription's payment method, allowing you to optimize for off-session payments. Learn more in the <a href="https://stripe.com/docs/billing/migration/strong-customer-authentication#scenario-2">SCA Migration Guide</a>.
 * @property null|\GiveDonationBlock\Strauss\Stripe\StripeObject $pending_update If specified, <a href="https://stripe.com/docs/billing/subscriptions/pending-updates">pending updates</a> that will be applied to the subscription once the <code>latest_invoice</code> has been paid.
 * @property null|string|\GiveDonationBlock\Strauss\Stripe\SubscriptionSchedule $schedule The schedule attached to the subscription
...
class Subscription extends ApiResource
{
     ...
     *
     * @return \GiveDonationBlock\Strauss\Stripe\Subscription the deleted subscription
     */
     ...
     * @throws \GiveDonationBlock\Strauss\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \GiveDonationBlock\Strauss\Stripe\Subscription the updated subscription
     */
